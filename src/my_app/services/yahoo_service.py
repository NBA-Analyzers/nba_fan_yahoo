...
from ..integrations.yahoo.sync_yahoo_league import YahooLeague
    ...
    def sync_league_data(
        ...
        try:
            ...
            try:
                ...
                yahoo_game = get_yahoo_sdk(self.token_store, {"user": user_guid})
                if not yahoo_game:
                    return {"success": False, "error": "Yahoo SDK not available"}
                ...
                # Step 6: Check if critical blobs succeeded
                critical_blobs = ["league_settings", "standings", "team_roster"]
                failed_critical = [
                    blob for blob in critical_blobs if not sync_results.get(blob, False)
                ]

                if failed_critical:
                    logger.error(
                        f"League {league_id}: Critical blobs failed: {failed_critical}"
                    )
                    return {
                        "success": False,
                        "message": f"Critical uploads failed: {', '.join(failed_critical)}",
                ...
                yahoo_league_repo.update_by_league_id(
                    league_id, {"last_blob_sync": datetime.utcnow().isoformat()}
                )

...
def yahoo_select_league():
    ...
    try:
        ...
        if teams:
            ...
            user_team_key = first_team_key
        
        # Get yahoo_user_id from session or token store
        user_guid = session.get('user')
        if not user_guid or user_guid not in token_store:
            return "User not authenticated", 401
        
        yahoo_user_id = token_store[user_guid].get('xoauth_yahoo_guid') or token_store[user_guid].get('guid')
        if not yahoo_user_id:
            return "Yahoo user ID not found", 400
        ...
        return f"<h2>League Sync Complete!</h2><p>{db_message}</p><p>Sync Results: {results}</p><br><a href='/dashboard'>← Back to Dashboard</a>"
...
@app.route('/yahoo/my_leagues')
@require_google_auth
def yahoo_my_leagues():
    ...
    try:
        ...
        yahoo_league_repo = YahooLeagueRepository()
        user_leagues = yahoo_league_repo.get_by_yahoo_user_id(yahoo_user_id)

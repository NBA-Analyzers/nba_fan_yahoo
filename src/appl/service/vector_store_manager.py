...
class VectorStoreManager:
    def __init__(self, vector_store_respository: VectorStoreMetadataRepository, openai_client: OpenAI):
        self.vector_store_meatadata_repository = vector_store_respository
        self.openai_client = openai_client

    def update_vector_store(self, vector_store_metadata_id: str, openai_file_ids: list[str]):
        openai_vector_store_id = self.create_vector_store_in_openai(vector_store_metadata_id, openai_file_ids)
        ...
        if vector_store_metadata is None:
            vector_store_metadata = VectorStoreMetadata(vector_store_id=vector_store_metadata_id)
    ...
    def create_vector_store_in_openai(self, vector_store_metadata_id: str, openai_file_ids: list[str]):
        vector_store = self.openai_client.vector_stores.create(name=vector_store_metadata_id)
    ...
    def get_vector_store_by_id(self, vector_store_id: str) -> Optional[VectorStoreMetadata]:
        return self.vector_store_meatadata_repository.get_by_vector_store_id(vector_store_id)

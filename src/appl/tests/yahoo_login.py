...
def callback():
    try:
        ...
        try:
            ...
            print(
                f"✅ Yahoo user successfully saved to database: {created_user.yahoo_user_id}"
            )
        except Exception as e:
        ...
        try:
            ...
            print(
                f"   - This user can now be linked to Google accounts through the connection process"
            )
...
def connect_yahoo_to_google():
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if not google_user:
                return (
                    f"❌ Google user with ID {google_user_id} not found. Please log in with Google first.",
                    400,
                )
            ...
            if not yahoo_user:
                return (
                    f"❌ Yahoo user with ID {yahoo_user_id} not found. Please log in with Yahoo first.",
                    400,
                )
            ...
            created_connection = fantasy_service.connect_fantasy_platform(
                google_fantasy
            )

...
class AzureBlobStorage:
    ...
    def __init__(
        ...
        container_name: str = "fantasy-league-data",
    ):
        """
        ...
        if not self.connection_string:
            raise ValueError(
                "Azure Storage connection string is required. Set AZURE_STORAGE_CONNECTION_STRING environment variable or pass it to constructor."
            )
    ...
    def upload_json_data(
        self, data: Dict[str, Any], blob_name: str, overwrite: bool = True
    ) -> bool:
        """
    ...
    def _compute_sha256(self, payload: bytes) -> str:
        """
        Compute the SHA-256 hash of the given payload.
    ...
    def _get_blob_sha256(self, blob_name: str) -> Optional[str]:
        """
        Retrieve the stored SHA-256 hash from a blob's metadata.

        This checks if a blob exists and reads its 'content_sha256' metadata field,
        which was previously stored during upload to track content changes.

        Args:
            blob_name: Name of the blob in the container

        Returns:
            The SHA-256 hash string if found, None if blob doesn't exist or has no hash metadata
        """
        try:
            blob_client = self.container_client.get_blob_client(blob_name)
            props = blob_client.get_blob_properties()
            if props.metadata and "content_sha256" in props.metadata:
                return props.metadata["content_sha256"]
            return None
        except ResourceNotFoundError:
            return None
        except Exception as e:
            logger.warning(f"Could not read metadata for '{blob_name}': {e}")
            return None
    ...
    def upload_json_with_retries(
        ...
        """
        Upload JSON with exponential backoff retries and content-hash short-circuit.
        Stores a 'content_sha256' metadata to skip unchanged content.

        Returns:
            bool: True if upload succeeded or was skipped (content unchanged), False if failed
        """
        try:
            payload = json.dumps(
        ...
        desired_sha = self._compute_sha256(payload)
        existing_sha = self._get_blob_sha256(blob_name)

        # Skip upload if content hasn't changed
        if existing_sha == desired_sha:
            logger.info(f"Skipping upload for '{blob_name}' (content unchanged)")
            return True

        # Content changed or blob doesn't exist - proceed with upload
        blob_client = self.container_client.get_blob_client(blob_name)
        backoff = 0.5
        attempts = 0
        ...
        while attempts < max_retries:
            ...
            try:
                blob_client.upload_blob(
                    payload, overwrite=True, metadata={"content_sha256": desired_sha}
                )
                logger.info(f"Uploaded '{blob_name}' (attempt {attempts})")
                return True
            except AzureError as e:
                logger.warning(
                    f"Upload attempt {attempts} failed for '{blob_name}': {e}"
                )
        ...
        logger.error(f"Exhausted retries uploading '{blob_name}'")
        return False

    def upload_file(
        self, file_path: str, blob_name: Optional[str] = None, overwrite: bool = True
    ) -> bool:
        """
        ...
        try:
            ...
            logger.info(
                f"Successfully uploaded file '{file_path}' to blob: {blob_name}"
            )
            return True

        except Exception as e:
            logger.error(
                f"Error uploading file '{file_path}' to blob '{blob_name}': {str(e)}"
            )
            return False
    ...
    def download_file(self, blob_name: str, local_file_path: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Successfully downloaded blob '{blob_name}' to '{local_file_path}' in current directory"
            )
            return True
            ...
            logger.error(
                f"Error downloading blob '{blob_name}' to '{local_file_path}': {str(e)}"
            )
            return False
    ...
    def list_blobs(self, name_starts_with: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            logger.info(
                f"Found {len(blob_names)} blobs in container '{self.container_name}'"
            )
            return blob_names
    ...
    def download_latest_league_data(
        self, league_id: str, data_type: str
        ...
        if not blobs:
            logger.warning(
                f"No data found for league '{league_id}' and type '{data_type}'"
            )
            return None
